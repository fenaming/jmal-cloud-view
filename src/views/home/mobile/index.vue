    ...
    <van-popup v-model="leftMenuShow" get-container="body" position="left" :style="{ height: '100%', width: '35%'}">
      <div class="left-menu">
        ...
        <div class="classification">
          ...
          <van-cell title="公共文件" icon="desktop-o" @click="clickFileType(null,{isTogether: true})"/>
<!--          <van-cell title="文件传输" icon="upgrade" @click="uploadList"/>-->
    ...
    <sim-text-preview :file.sync="textPreviewRow" :status.sync="textPreviewVisible" :is-together.sync="isTogether"></sim-text-preview>
    ...
    <office-preview :file="officePreviewRow" :status.sync="officePreviewVisible" :is-together.sync="isTogether"></office-preview>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      queryFileType: null,
      queryCondition: {isFolder: null, isTogether: false},
      loading: false,
      finished: false,
      refreshing: false,
  ...
  methods: {
    ...
    favoriteOperating(isFavorite) {
      if (this.isTogether){
        vant.Toast.setDefaultOptions({duration: 1200});
        vant.Toast('公共文件无法收藏');
        // vant.Toast.warning('公共文件无法收藏');
        // Notify({type: 'warning', message: '公共文件无法收藏', duration: 1000});
      }else{
    ...
    clickFileType(queryFileType, queryCondition) {
      ...
      if (this.queryCondition.isTogether){
