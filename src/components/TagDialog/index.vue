...
export default {
  ...
  props: {
    ...
    // 允许外部传入 API 对象，用于在不同空间（个人/共享/图库）调用对应的 setTag 端点
    api: {
      type: Object,
      default: () => null
    }
  ...
  methods: {
    ...
    submitTag() {
      ...
      const currentApi = this.api || fileApi
      currentApi.setTags({fileIds: fileIdList, tagList: this.tags, removeTagIds: this.remoteTagIdList}).then(() => {
