      ...
      <div slot="title" class="simtext-header-title">
        ...
        <div class="title-extension">
          ...
          <history-popover
            ...
            :is-together.sync="this.$props.isTogether"
            :space-flag="this.$props.spaceFlag"
            buttonSize="small"
  ...
  export default {
    ...
    watch: { //监听file的变化，进行相应的操作即可
      file(file) {
        ...
        if (this.$props.isTogether){
          togetherApi[request](params).then((res) => {
            this.loading.close()
        ...
        }else if(this.$props.spaceFlag){
          params.spaceFlag = this.$props.spaceFlag
          spaceApi[request](params).then((res) => {
            this.loading.close()
            this.directoryTreeData = {
              name: res.data.name,
              isFolder: true,
    ...
    methods:{
      ...
      async requestStream(request, params, index) {
        try {
          ...
          let url = ""
          if (this.$props.isTogether){
            url = "/api/together/preview/text/stream"
          }else if(this.$props.spaceFlag){
            url = "/api/space/preview/text/stream"
          }else{
            url = "/api/preview/text/stream"
          }
          if (request === 'previewTextByPath') {
            if (this.$props.isTogether){
              url = "/api/together/preview/path/text/stream"
            }else if(this.$props.spaceFlag){
              url = "/api/space/preview/path/text/stream"
            }else{
      ...
      treeNodeClick(row) {
        ...
        if(this.editableTabs.findIndex(tab=> tab.name===row.path) < 0){
          ...
          if(this.$props.isTogether){
            togetherApi.previewTextByPath(params).then(()=>{
              this.loading.close()
              // 添加一个tab
              let tab = {
                title:row.name,
            ...
            params.spaceFlag = this.$props.spaceFlag
            spaceApi.previewTextByPath(params).then(()=>{
              this.loading.close()
              // 添加一个tab
              let tab = {
                title:row.name,
      ...
      update(value,path,index) {
        ...
        markdownApi.editMarkdownByPath({
            ...
            spaceFlag: this.$props.spaceFlag,
          }).then(() => {
            this.updating = false
            if(undefined !== index){
