      ...
      if (visible) {
        ...
        if (this.file.m3u8) {
          if (this.file.isTogether){
            videoUrl = `${fileConfig.baseUrl}/together/video/hls/${this.file.m3u8}`
            if (this.shareId) {
              const shareToken = this.$store.getters.shareToken ? this.$store.getters.shareToken : 'token'
              videoUrl = `${fileConfig.baseUrl}/together/public/video/hls/${window.shareId}/${shareToken}/${this.file.m3u8}`
            }
          }else{
