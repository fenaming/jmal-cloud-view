  ...
  <div>
    ...
    <div class="office-preview" v-if="show" v-loading="!readyShow">
        <div class="preview-block" v-show="readyShow">
          ...
          <div class="wrapper">
            <pdf-preview v-if="fileType === 'pdf' || fileType === 'PDF'" :file="file" :shareId="shareId" :file-url="fileUrl" @onReady="onReady"></pdf-preview>
            <drawio v-else-if="fileType === 'drawio'" v-show="fileReday" :file="file" :shareId="shareId" :read-only="readOnly" :is-together.sync="this.$props.isTogether" :space-flag="this.$props.spaceFlag" @onEdit="onEdit" @onReady="onReady" @onClose="close"></drawio>
            <my-mind-editor v-else-if="fileType === 'mind'" :file="file" :shareId="shareId" :read-only="readOnly" :is-together.sync="this.$props.isTogether" :space-flag="this.$props.spaceFlag" @onEdit="onEdit" @onReady="onReady" @onClose="close"></my-mind-editor>
...
export default {
  ...
  methods: {
     ...
     */
    checkReadOnly(file){
      if((this.$store.state.user.token && this.$store.state.user.userId === file.userId) || file.spaceFlag || file.isTogether){
        this.readOnly = false
      }
