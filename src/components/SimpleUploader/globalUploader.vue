...
export default {
  ...
  data() {
    return {
      ...
      options: {
        // target 和 query 使用函数形式，从每个文件独立的 _uploadContext 中读取
        // 避免全局 opts 被覆盖导致不同空间的文件上传到错误端点
        target: function(file) {
          const context = file._uploadContext
          if (context && context.api) {
            return context.api.simpleUploadURL
          }
          return fileApi.simpleUploadURL
        },
        ...
        query: function(file) {
          ...
          if (context && context.params) {
            return { isFolder: false, ...context.params }
          }
          return {}
        }
      },
  ...
  watch: {
    $route(route) {
      if (route.path === '/'){
        this.enableDragUplaod = true
        this.isTogether = false
        this.spaceFlag = ''
      }else if (route.path.replace(/\//g, "") === 'together'){
        this.enableDragUplaod = true
        this.isTogether = true
        this.spaceFlag = ''
      }else if (route.path.replace(/\//g, "") === 'designer' || route.path.replace(/\//g, "") === 'machine'
        || route.path.replace(/\//g, "") === 'install' || route.path.replace(/\//g, "") === 'control'
        || route.path.replace(/\//g, "") === 'electric' || route.path.replace(/\//g, "") === 'sample'
        || route.path.replace(/\//g, "").indexOf('space') > -1){
        this.enableDragUplaod = true
        this.isTogether = false
        this.spaceFlag = route.path.replace(/\//g, "")
      }else{
        this.enableDragUplaod = false
        this.isTogether = false
        this.spaceFlag = ''
      }
    },
  ...
  mounted() {
    if (this.$route.path === '/'){
      ...
      this.isTogether = false
      this.spaceFlag = ''
    }else if (this.$route.path.replace(/\//g, "") === 'together'){
      this.enableDragUplaod = true
      this.isTogether = true
      this.spaceFlag = ''
    }else if (this.$route.path.replace(/\//g, "") === 'designer' || this.$route.path.replace(/\//g, "") === 'machine'
      || this.$route.path.replace(/\//g, "") === 'install' || this.$route.path.replace(/\//g, "") === 'control'
      || this.$route.path.replace(/\//g, "") === 'electric' || this.$route.path.replace(/\//g, "") === 'sample'
      || this.$route.path.replace(/\//g, "").indexOf('space') > -1){
      this.enableDragUplaod = true
      this.isTogether = false
      this.spaceFlag = this.$route.path.replace(/\//g, "")
    }else{
      this.enableDragUplaod = false
      this.isTogether = false
      this.spaceFlag = ''
  ...
  methods: {
    ...
    onFilesAdded(files) {
      ...
      if (pathsLength > 0) {
        paths.forEach(path => {
          ...
          currentApi.uploadFolder({
            ...
            isTogether: this.params.isTogether,
            spaceFlag: this.params.spaceFlag
          }).then(() => {
            store.dispatch('updateMessage', {event: 'fileSuccess', data: null})
          }).catch(e => {
          })
        })
      }
      ...
      // 为每个文件保存独立的上传上下文（API和参数快照），
      // 避免路由切换或多空间并发上传时参数被覆盖导致文件上传到错误空间
      const uploadContext = {
        api: currentApi,
        params: { ...this.params }
      }
      files.forEach(file => {
        file._uploadContext = uploadContext
    ...
    onFileSuccess(rootFile, file, response) {
      ...
      if (data.merge) {
        ...
        // 使用文件独立的上传上下文，避免路由切换后 merge 请求发到错误的 API 端点
        const ctx = file._uploadContext || { api: this.getCurrentApi(), params: this.params }
        ctx.api.mergeSimpleUpload({
          filename: encodeIfNeeded(file.name),
          relativePath: encodeIfNeeded(file.relativePath),
          identifier: file.uniqueIdentifier,
          folder: ctx.params.folder || this.$route.query.folder,
          currentDirectory: encodeIfNeeded(ctx.params.currentDirectory),
          username: ctx.params.username,
          userId: ctx.params.userId,
          totalSize: file.size,
          isFolder: file.isFolder,
          isTogether: ctx.params.isTogether,
          spaceFlag: ctx.params.spaceFlag
        }).then(() => {
    ...
    computeMD5Success(md5, file) {
      // 不再覆盖全局 uploader.opts.target/query，使用文件独立的 _uploadContext
      // target 和 query 已在 options 中定义为函数，会自动从 file._uploadContext 读取
