...
<!--            </pl-table-column>-->
    ...
    <sim-text-preview
      ...
      :is-together.sync="isTogether"
      :space-flag="spaceFlag"
    ></sim-text-preview>
    <image-viewer
      :fileList="fileList"
    ...
    ></office-preview>
    <cad-preview
      v-if="cadPreviewVisible"
      :status.sync="cadPreviewVisible"
      :file="cadPreviewRow"
      :is-together.sync="isTogether"
      :space-flag="spaceFlag"
    ...
    <tag-dialog :fileList="tagDialogObjectList"
                :status.sync="tagDialogVisible" :api="api" @onSuccess="allocateTagSuccess"/>
    <share-dialog
...
export default {
  ...
  data() {
    return {
      ...
      onCreateFilename: '',
      isTogether: false,
      // spaceFlag: this.$route.path.replaceAll("/", ""),
      spaceFlag: this.$route.path.replace(/\//g, ""),
      api: api, // 暴露给模板，供 tag-dialog 组件使用
    };
  },
  computed: {
    ...mapState(["message"]),
  ...
  methods: {
    ...
    downloadFile() {
      ...
      if (totalSize > 0) {
        ...
        if (fileIds.length > 1 || this.rowContextData.isFolder) {
          fileConfig.packageDownloadSpace(
            fileIds,
            this.$store.state.user.token,
    ...
    fileClick(row) {
      ...
      if (row.isFolder) {
        ...
        if (suffix.cadFile.includes(row.suffix.toUpperCase())) {
          // 图纸文件
          this.cadPreviewRow = row;
          this.cadPreviewVisible = true;
          return;
        }
        ...
        if (
          ...
          ["pdf", "PDF", "csv", "drawio", "mind"].includes(row.suffix)
        ) {
          // office文件
          this.officePreviewVisible = true;
    ...
    closeHandle () {
      this.cadPreviewVisible = false  // 控制取消和X按钮，关闭弹窗
    }
  }
};
</script>
