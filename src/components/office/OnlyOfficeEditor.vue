      ...
      handler(id)  {
        ...
        $J.loadScript($J.apiUrl(officeApiUrl), (e) => {
          if (e !== null) {
            ...
            if (this.file.isTogether){
              togetherApi.getFileInfoById({id: this.file.id}).then(res => {
                this.file = res.data
                this.loadFile()
              })
            }else if(this.file.spaceFlag){
              spaceApi.getFileInfoById({id: this.file.id, spaceFlag:this.file.spaceFlag}).then(res => {
                this.file = res.data
                this.loadFile()
              })
            }else if(this.$store.state.user.token && this.$store.state.user.userId === this.file.userId){
