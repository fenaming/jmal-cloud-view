    ...
    methods: {
      ...
      lazyLoadData(data){
        ...
        return new Promise(function(resolve) {
            ...
            }else if(that.$props.spaceFlag){

              spaceApi.listfiles({
                username: that.$store.state.user.name,
                path: encodeURI(data.path),
                spaceFlag: that.$props.spaceFlag
              }).then(res => {
                resolve(that.convert(res.data))
              })
            }else{
